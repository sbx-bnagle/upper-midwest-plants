...
.query-pane{max-height:0;overflow:hidden;transition:max-height .22s ease}
.query-pane.open{max-height:200px}
.qinner{display:flex;flex-wrap:wrap;gap:10px 16px;align-items:flex-end;
...
/* Sticky single-row thead.
   ALL ths get identical padding — this sets a consistent row height.
   The Season th adds position:relative so tlhead can be absolutely placed
   within the same bottom-padding space. */
...
thead .cols th{
  ...
  /* Extra bottom padding = space below label used by Season's month markers */
...
/* Season th: month labels sit inside the bottom-padding zone */
...
/* Single vertical rule — only left of Width column */
...
/* Character: category (capitalize) on top, tier below — same size */
...
<div class="controls" id="controls">
  <div class="controls-bar">
    ...
    <button class="qbtn" id="query-toggle">Filter <span id="qt-arrow">&#9662;</span></button>
...
<thead>
<!-- Single row; all labels share the same cell height.
     Month labels sit in the Season th's bottom-padding zone (position:absolute). -->
<tr class="cols">
  <th class="s" data-k="com">Name <span class="ar"></span></th>
  <th class="s" data-k="tier_group">Character <span class="ar"></span></th>
  <th class="s" data-k="soil">Soil <span class="ar"></span></th>
...
<footer><b>Width &amp; Height</b> ft ranges. <b>Season</b> bar marks bloom months on a Mar&ndash;Feb axis, filled with bloom color when known. <b>Character</b> lists plant category and nativity tier. <b>Light</b> and <b>Water</b> icons show level; hover to see source text. Click any row for full detail.</footer>
</div>
...
/* ── Sun icon — single flat color per level ──────────────────────────────── */
...
function sunSVG(level,title){
  const sz=22,cx=11,cy=11,r=4.5;
  /* Single color per level — no separate fill/stroke split */
  const col={1:'#aba49a',2:'#23211b',3:'#23211b',4:'#d4a010'}[level]||'#aba49a';
  const numRays={1:0,2:3,3:5,4:8}[level]||0;
  const ri=6.8, ro=10.5;
  let g='';
  for(let i=0;i<numRays;i++){
    const a=(i*360/numRays-90)*Math.PI/180;
    const x1=(cx+ri*Math.cos(a)).toFixed(2),y1=(cy+ri*Math.sin(a)).toFixed(2);
    const x2=(cx+ro*Math.cos(a)).toFixed(2),y2=(cy+ro*Math.sin(a)).toFixed(2);
    g+=`<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${col}" stroke-width="1.8" stroke-linecap="round"/>`;}
  /* Filled circle only — no separate outline stroke */
  return`<svg width="${sz}" height="${sz}" viewBox="0 0 ${sz} ${sz}" style="display:inline-block;vertical-align:middle"><title>${title}</title>${g}<circle cx="${cx}" cy="${cy}" r="${r}" fill="${col}"/></svg>`;}

/* ── Water drops — 5 drops in 3+2 rows, N dark + (5-N) light ─────────────
   Shape from reference: wide rounded base, soft pointed top.
   ViewBox 9×13: tip at (4.5,0.5), circle center at (4.5,8.5), radius 4. */
...
/* Drop path — 9×12 viewBox, 0.78 w/h ratio matching reference.
   Arc sweep-flag=1 (clockwise) → rounded bottom at (4.5,12). */
const DP='M4.5,.5C2.5,2,0,5,0,7.5A4.5,4.5,0,0,1,9,7.5C9,5,6.5,2,4.5,.5Z';
function oneDrop(filled){
  const ink='#23211b', lite='rgba(35,33,27,.18)';
  return`<svg width="9" height="12" viewBox="0 0 9 12"><path d="${DP}" fill="${filled?ink:lite}"/></svg>`;}
function waterSVG(level,title){
  /* Row 1: drops 1-3 | Row 2: drops 4-5 (centered under row 1) */
  let r1='',r2='';
  for(let i=1;i<=3;i++)r1+=oneDrop(i<=level);
  for(let i=4;i<=5;i++)r2+=oneDrop(i<=level);
  return`<span title="${title}" style="display:inline-flex;flex-direction:column;align-items:center;gap:2px;line-height:1">`+
    `<span style="display:flex;gap:2px">${r1}</span>`+
    `<span style="display:flex;gap:2px">${r2}</span></span>`;}
...
function rowEl(p){
  ...
  const tier=esc(p.tier_group)||'', cat=esc(p[K.cat])||'';
  /* Character: category on top (capitalize), tier below */
  ...
  tr.innerHTML=
    ...
    charCell+
    `<td class="req">${esc(p[K.soil])||'<span class="empty-c">&middot;</span>'}</td>`+
    `<td class="icon-cell">${lightLv?sunSVG(lightLv,p[K.light]):'<span class="empty-c">&middot;</span>'}</td>`+
    `<td class="icon-cell">${waterLv?waterSVG(waterLv,p[K.water]):'<span class="empty-c">&middot;</span>'}</td>`+
...
function apply(){
  ...
  document.querySelectorAll('thead .cols th.s').forEach(th=>{
    ...
    a.textContent=th.dataset.k===state.sort?(state.dir>0?'\u25B2':'\u25BC'):'';});
  more();}
function more(){
